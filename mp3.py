...
class Mp3Player(QMainWindow, Ui_MainWindow):
    def __init__(self):
        ...
        self.pushButton_10.clicked.connect(self.minus_ten_sec)
        self.pushButton_11.clicked.connect(self.plus_ten_sec)
    ...
    def play_func(self):
        if not self.is_stoped and self.songs:
            ...
            if self.play == 0:
                ...
                self.label_2.setText(self.currsong)
                self.timer.start(1000)
    ...
    def stop(self):
        if self.songs:
            ...
            self.msc.set_volume(self.currvol)
            self.lcdNumber.display('00:00')
            self.currsong, self.currsong_ind = self.songs[0], 0
            self.label_2.setText('')
            self.is_stoped = True
            self.horizontalSlider_2.setSliderPosition(0)
            self.pushButton_2.setIcon(self.style().standardIcon(QStyle.SP_MediaPlay))
    ...
    def download(self):
        self.dl = QFileDialog.getOpenFileName(self, 'Загрузить песню', '', '*.mp3')[0].split('/')[-1]
        if self.dl != '':
            self.songs.append(self.dl)
            self.listWidget.addItem(self.dl)
    ...
    def other_song(self, item):
        if not self.is_stoped:
            ...
            self.timer.stop()
            self.play = 0
            self.currsong_ind = self.songs.index(it)
            self.currsong = self.songs[self.currsong_ind]
    ...
    def plus_ten_sec(self):
        if self.songs:
            self.msc.stop()
            self.msc.load(self.currsong)
            self.msc.play(start=self.count_for_timer + 10)
            self.count_for_timer += 10

    def minus_ten_sec(self):
        if self.songs:
            if self.count_for_timer - 10 > 0:
                self.msc.stop()
                self.msc.load(self.currsong)
                self.msc.play(start=self.count_for_timer - 10)
                self.count_for_timer -= 10
...
# TODO pyinstaller не поддерживает кириллицу
